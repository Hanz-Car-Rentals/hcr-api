...
db.query("SELECT * FROM roles", function (err, result) {
  ...
  if (result.length === 0) {
      ...
      db.query("INSERT INTO roles (role_name, role_desc, role_level) VALUES (?, ?, ?)", ['Voertuigbeheerders', 'Medewerkers die ervoor zorgen dat de auto\'s in orde zijn. Ze regelen reparaties en zorgen dat alle papieren in orde zijn.', 2]);
      ...
      db.query("INSERT INTO roles (role_name, role_desc, role_level) VALUES (?, ?, ?)", ['Gebruiker', 'Mensen die een auto willen huren. Ze zoeken naar een auto, boeken er een en betalen online.', 4]);
  }
});
...
db.query("SELECT * FROM users", function (err, result) {
  ...
  if (result.length === 0) {
      ...
      crypto.pbkdf2(password, salt, 310000, 32, 'sha256', function(err, hashedPassword) {
          ...
          const hashedPasswordHex = hashedPassword.toString('hex'); // Convert hashed password to hexadecimal string
          ...
          db.query("INSERT INTO salts (salt) VALUES (?)", [salt], function (err, result) {
              ...
              // Insert the user into the users table with the salt ID and hashed password
              db.query("INSERT INTO users (first_name, last_name, email, email_verified, password, salt, role) VALUES (?, ?, ?, ?, ?, ?, ?)",
                  ['Hcr', 'Admin', 'hcr.admin@wolfsoft.solutions', 1, hashedPasswordHex, saltId, 1],
                  function (err, result) {
                      if (err) throw err;
