    ...
    db.query('SELECT * FROM users WHERE email = ?', [email], function (error, results, fields) {
        if (error) {
            ...
            if (results.length < 1) {
                ...
                let user = results[0];
                db.query("SELECT * FROM salts WHERE id = ?", [user.salt], function (error, results, fields) {
                    if(error){
                        send_error(error, "Error logging in");
                        throw error;
                    };
                    ...
                    crypto.pbkdf2(password, salt, 310000, 32, 'sha256', function(err, hashedPassword) {
                        if (hashedPassword.toString('hex') == user.password.toString('hex')) {
                            ...
                            db.query('UPDATE users SET token =?, token_expires_at = DATE_ADD(NOW(), INTERVAL 3 HOUR) WHERE id =?', [token, user.id], function (err, rows) {
                                if(err) {
                                    send_error(err, 'Updating token');
                                    throw err;
                                };
                                // get the role info 
                                db.query('SELECT * FROM roles WHERE id =?', [user.role], function (error, results, fields) {
                                    if(error){
                                        send_error(error, "Error logging in");
                                        throw error;
                                    };
                                    ...
                                    let role = results[0];
                                    res.send({'status':200,'message': 'Successfully logged in', "user": { "first_name": user.first_name, "last_name": user.last_name, 'token': token, 'role': role.role_name }});
                                });
                            });
                        } else {
                            res.send({'status':401,'message': 'Invalid email or password'});
...
// mauro@wolfsoft.solutions token: 35080f5a28b0deb1786eeeaff3cff87d
// hcr.admin@wolfsoft.solutions token: ba82fed18e109f57414ac0612ce9e5ea

// users route to get all users as admin user
router.get('/', check_user_token, admin_check, function (req, res) {
    db.query('SELECT id,first_name,last_name,email,email_verified,role,verified_drivers_licence,times_rented,currently_renting,created_at,updated_at FROM users', function (error, results, fields) {
        if (error) {
            send_error(error, "Error getting users");
            res.send({'status':500,'message': 'Error getting users'});
        } else {
            res.send({'status':200,'message': 'Successfully got users', 'users': results});
        }
    });
});
...
router.get('/user/:id', check_user_token, user_check, function (req, res) {
    db.query('SELECT id,first_name,last_name,email,email_verified,role,verified_drivers_licence,times_rented,currently_renting,created_at,updated_at FROM users WHERE id =?', [req.params.id], function (error, results, fields) {
        if (error) {
            send_error(error, "Error getting user");
            res.send({'status':500,'message': 'Error getting user'});
        } else {
            // get the role info from the roles table:
            db.query('SELECT * FROM roles WHERE id =?', [results[0].role], function (error, role_results, fields) {
                if(error){
                    send_error(error, "Error getting user");
                    throw error;
                };
                ...
                if(results[0].currently_renting === null){
                    ...
                    db.query('SELECT * FROM cars WHERE id =?', [results[0].currently_renting], function (error, car_results, fields) {
                        if(error){
                            send_error(error, "Error getting user");
                            throw error;
                        };
                ...
                res.send({'status':200,'message': 'Successfully got user', 'user': results[0]});
            });
        }
    });
});
